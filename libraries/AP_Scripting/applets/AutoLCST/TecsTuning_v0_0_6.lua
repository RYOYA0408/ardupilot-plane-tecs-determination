...
function tecstuning(sw)
    ...
    if tecstune_stage == 2 then
        ...
        if climb_sw and height_now >= height_climb_min and throttle_now == 100 then
            ...
            if #arspd_val_buff >= FREQUENCY + 1 then   -- 最低1秒間の幅
                local dt = 1.0
                local dV = arspd_val_buff[#arspd_val_buff] - arspd_val_buff[#arspd_val_buff - FREQUENCY]
            ...
            if not get_climb_rate_sw and #arspd_buff == buff_size then
                ...
                if all_ok then
                    ...
                    param:set_and_save("SCR_USER3", pitch_max)
                    param:set_and_save("SCR_USER4", pitch_min)
    ...
    if tecstune_stage == 3 then
        ...
        local arspd_err = math.abs(arspd_now - arspd_target)    -- 誤差監視
        param:set_and_save("TECS_SPDWEIGHT", 0)
        local spd_weight = param:get("TECS_SPDWEIGHT")
        gcs:send_text(0, string.format("TECS_SPDWEIGHT :", spd_weight))
        ...
        if not get_sink_min_sw then
            ...
            -- dh履歴をバッファに記録&サイズを超えたら古いデータから削除
            table.insert(dh_val_buff, dh_now)
            if #dh_val_buff > buff_size then
                table.remove(dh_val_buff, 1)
            end

            -- dhの変化幅をチェック
            local dh_var_ok = false
            if #dh_val_buff == buff_size then
                local max_dh, min_dh = dh_val_buff[1], dh_val_buff[1]
                for _, dh in ipairs(dh_val_buff) do
                    if dh > max_dh then max_dh = dh end
                    if dh < min_dh then min_dh = dh end
                end
                dh_var_ok = (max_dh - min_dh) <= dh_margin
            end

            -- 上記の条件をすべて満足したとき最小降下率を取得
            if arspd_err <= arspd_margin and slope_ok and throttle_now == thr_min and dh_var_ok then
