...
bool Plane::verify_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (auto_state.crosstrack) {
        if (auto_state.checked_for_autoland) {
            ...
            flex_next_WP_loc.set_alt_cm(h*100, AltFrame::ABOVE_HOME);
        }
...
bool Plane::verify_nav_tp(const AP_Mission::Mission_Command& cmd)
{
    ...
    // Turning point 円周上を飛行中
        ...
        if (auto_state.checked_for_autoland) {
            ...
            flex_prev_WP_loc.set_alt_cm(h*100, AltFrame::ABOVE_HOME);
        ...
        if (tp_dist <= acceptance_distance_m && c1) {       // 目標点への近接判定
            ...
            if (auto_state.checked_for_autoland) {
                auto_state.rtl_land_seq_lastwp_distance += 2.0*radius*loiter.total_cd/36000.0*M_PI;
            }
            auto_state.tp_circle_mode = 0;
