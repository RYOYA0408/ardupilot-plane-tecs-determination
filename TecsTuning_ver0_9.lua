...
local thr_min_time = nil
local thr_min_sw = false
...
function tune_tecs(sw)
    ...
    if tecstune_stage == 3 then
        if not get_sink_min_sw then
            if not thr_min_sw and throttle_now == thr_min then
                thr_min_time = millis():tofloat()   -- スロットルが最小になった時を記録
                thr_min_sw = true
            end

            local now = millis():tofloat()  -- 現在の時刻を記録
            if thr_min_sw and (now - thr_min_time) >= 4000 and arspd_error <= arspd_margin then
